...
describe("/api/sessions/[sessionId]/code-editor", () => {
  ...
  test("POST returns 403 for managed-template trial users", async () => {
    ...
    const { POST } = await routeModulePromise;
    const expectedError =
      "The code editor is disabled in the hosted demo. Deploy your own copy to unlock the full Agents OSS template.";

    const response = await POST(
      new Request(
        "https://agents-oss.vercel.app/api/sessions/session-1/code-editor",
        {
          method: "POST",
        },
      ),
