...
describe("linkHarnessWorkingDirectory", () => {
  test("links the external harness directory to the Open Agents workspace", async () => {
    ...
    await linkHarnessWorkingDirectory({
      session,
      sessionWorkDir: "/tmp/agents-oss-harness/codex-session-1",
      workingDirectory: "/vercel/sandbox",
    });

    expect(run).toHaveBeenCalledWith({
      command:
        "rm -rf -- '/tmp/agents-oss-harness/codex-session-1' && ln -s -- '/vercel/sandbox' '/tmp/agents-oss-harness/codex-session-1'",
